/* ----------------------------------------------------------------------------
 * This file was automatically generated by SWIG (https://www.swig.org).
 * Version 4.1.1
 *
 * Do not make changes to this file unless you know what you are doing - modify
 * the SWIG interface file instead.
 * ----------------------------------------------------------------------------- */

package com.example;

public class ffi_lib {
  public static vec2 my_function(vec2 input) {
    return new vec2(ffi_libJNI.my_function(vec2.getCPtr(input), input), true);
  }

  public static vec3 vec3_new(int x, int y, int z) {
    long cPtr = ffi_libJNI.vec3_new(x, y, z);
    return (cPtr == 0) ? null : new vec3(cPtr, false);
  }

  public static int add(vec3 slf, int x, int y, int z) {
    return ffi_libJNI.add(vec3.getCPtr(slf), slf, x, y, z);
  }

  public static int add_reverse_args(int x, int y, int z, vec3 slf) {
    return ffi_libJNI.add_reverse_args(x, y, z, vec3.getCPtr(slf), slf);
  }

  public static int dot(vec3 slf, vec3 other) {
    return ffi_libJNI.dot(vec3.getCPtr(slf), slf, vec3.getCPtr(other), other);
  }

  public static vec3 cross(vec3 slf, vec3 other) {
    return new vec3(ffi_libJNI.cross(vec3.getCPtr(slf), slf, vec3.getCPtr(other), other), true);
  }

  public static vec3 normalize(vec3 slf) {
    return new vec3(ffi_libJNI.normalize(vec3.getCPtr(slf), slf), true);
  }

  public static void hello(String name) {
    ffi_libJNI.hello(name);
  }

  public static double benchmark_rust() {
    return ffi_libJNI.benchmark_rust();
  }

  public static double benchmark_rust_async() {
    return ffi_libJNI.benchmark_rust_async();
  }

  public static ffierror simple_service_destroy(SWIGTYPE_p_p_simpleservice context) {
    return ffierror.swigToEnum(ffi_libJNI.simple_service_destroy(SWIGTYPE_p_p_simpleservice.getCPtr(context)));
  }

  public static ffierror simple_service_new_with(SWIGTYPE_p_p_simpleservice context, long some_value) {
    return ffierror.swigToEnum(ffi_libJNI.simple_service_new_with(SWIGTYPE_p_p_simpleservice.getCPtr(context), some_value));
  }

  public static ffierror vec4_destroy(SWIGTYPE_p_p_vec4 context) {
    return ffierror.swigToEnum(ffi_libJNI.vec4_destroy(SWIGTYPE_p_p_vec4.getCPtr(context)));
  }

  public static ffierror vec4_new(SWIGTYPE_p_p_vec4 context, float x, float y, float z, float w) {
    return ffierror.swigToEnum(ffi_libJNI.vec4_new(SWIGTYPE_p_p_vec4.getCPtr(context), x, y, z, w));
  }

  public static float vec4_dot(SWIGTYPE_p_vec4 context, SWIGTYPE_p_vec4 other) {
    return ffi_libJNI.vec4_dot(SWIGTYPE_p_vec4.getCPtr(context), SWIGTYPE_p_vec4.getCPtr(other));
  }

  public static float vec4_get_x(SWIGTYPE_p_vec4 context) {
    return ffi_libJNI.vec4_get_x(SWIGTYPE_p_vec4.getCPtr(context));
  }

  public static float vec4_get_y(SWIGTYPE_p_vec4 context) {
    return ffi_libJNI.vec4_get_y(SWIGTYPE_p_vec4.getCPtr(context));
  }

  public static float vec4_get_z(SWIGTYPE_p_vec4 context) {
    return ffi_libJNI.vec4_get_z(SWIGTYPE_p_vec4.getCPtr(context));
  }

  public static float vec4_get_w(SWIGTYPE_p_vec4 context) {
    return ffi_libJNI.vec4_get_w(SWIGTYPE_p_vec4.getCPtr(context));
  }

  public static void vec4_set_x(SWIGTYPE_p_vec4 context, float value) {
    ffi_libJNI.vec4_set_x(SWIGTYPE_p_vec4.getCPtr(context), value);
  }

  public static void vec4_set_y(SWIGTYPE_p_vec4 context, float value) {
    ffi_libJNI.vec4_set_y(SWIGTYPE_p_vec4.getCPtr(context), value);
  }

  public static void vec4_set_z(SWIGTYPE_p_vec4 context, float value) {
    ffi_libJNI.vec4_set_z(SWIGTYPE_p_vec4.getCPtr(context), value);
  }

  public static void vec4_set_w(SWIGTYPE_p_vec4 context, float value) {
    ffi_libJNI.vec4_set_w(SWIGTYPE_p_vec4.getCPtr(context), value);
  }

  public static void init_logger() {
    ffi_libJNI.init_logger();
  }

}
